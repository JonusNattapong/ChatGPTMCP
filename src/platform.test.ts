...
test('bash tunnel scripts pass syntax validation when bash is available', (t) => {
  ...
  for (const name of ['install.sh', 'start-tunnel.sh', 'status-tunnel.sh', 'stop-tunnel.sh', 'refresh-tunnel.sh']) {
...
test('PowerShell tunnel scripts parse on Windows', (t) => {
  ...
  for (const name of ['install.ps1', 'start-tunnel.ps1', 'status-tunnel.ps1', 'stop-tunnel.ps1', 'refresh-tunnel.ps1']) {
