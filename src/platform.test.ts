...
test('bash tunnel scripts pass syntax validation when bash is available', (t) => {
  ...
  for (const name of ['start-tunnel.sh', 'status-tunnel.sh', 'stop-tunnel.sh', 'refresh-tunnel.sh']) {
    const result = spawnSync('bash', ['-n', path.join(projectRoot, 'scripts', name)], { encoding: 'utf8' });
...
test('PowerShell tunnel scripts parse on Windows', (t) => {
  ...
  for (const name of ['start-tunnel.ps1', 'status-tunnel.ps1', 'stop-tunnel.ps1', 'refresh-tunnel.ps1']) {
